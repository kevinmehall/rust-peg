...
pub(crate) fn compile_grammar(grammar: &Grammar) -> TokenStream {
    ...
    let context = &Context {
        ...
        parse_state_ty: quote!(&mut ParseState<'input, '_ #(, #grammar_lifetime_params)*>),
        extra_args_call,
        extra_args_def,
        injected_vars,
    };
...
fn make_parse_state(grammar: &Grammar) -> TokenStream {
    ...
    let grammar_lifetime_params = ty_params_slice(&grammar.lifetime_params);
    let grammar_lifetime_bounds = ty_params_slice(&grammar.lifetime_bounds);
    let mut cache_fields_tys: Vec<TokenStream> = Vec::new();
    ...
    for rule in grammar.iter_rules() {
        if rule.cache.is_some() && rule.cacheable() {
            ...
            let cache_ty = if rule.params.is_empty() {
                quote_spanned!(span=> usize)
            } else {
                let cache_ty_name = format_ident!("Cached{}", rule.name);
                let param_ty = rule.params.iter().map(|param| {
                    let RuleParamTy::Rust(ty) = &param.ty else {
                        unreachable!()
                    };
                    quote_spanned!(span=> <::peg::chomp_ref!(#ty) as ::peg::Cacheable>::Cached)
                });
                let borrowed_ty = rule.params.iter().map(|param| {
                    let RuleParamTy::Rust(ty) = &param.ty else {
                        unreachable!()
                    };
                    ty
                });
                let lhs = (1..=rule.params.len()).map(Literal::usize_unsuffixed);
                let rhs = lhs.clone();
                let cache_ty = quote_spanned!(span=> #cache_ty_name<'input, 'rule #(, #grammar_lifetime_params)*>);
                cache_fields_tys.push(quote_spanned!{span=>
                    #[allow(non_camel_case_types)]
                    #[automatically_derived]
                    #[derive(::core::cmp::Eq, ::core::hash::Hash, ::core::cmp::PartialEq)]
                    struct #cache_ty_name<'input: 'rule, 'rule #(, #grammar_lifetime_bounds)*>(usize #(, #param_ty)*, ::core::marker::PhantomData<(&'input (), &'rule () #(, &#grammar_lifetime_params ())*)>);
                    impl<'input: 'rule, 'rule #(, #grammar_lifetime_bounds)*> ::peg::hashbrown::Equivalent<#cache_ty> for (usize #(, #borrowed_ty)*) {
                        fn equivalent(&self, key: &#cache_ty) -> bool {
                            self.0 == key.0 #(&& ::peg::Cacheable::key(&self.#lhs).equivalent(&key.#rhs))*
                        }
                    }
                });
                cache_ty
            ...
            cache_fields_def.push(
                quote_spanned!(span=> #name: ::peg::hashbrown::HashMap<#cache_ty, ::peg::RuleResult<#ret_ty>>),
            );
    ...
    quote_spanned! { span =>
        ...
        #[allow(unused_parens)]
        struct ParseState<'input: 'rule, 'rule #(, #grammar_lifetime_bounds)*> {
            _phantom: ::core::marker::PhantomData<(&'input (), &'rule () #(, &#grammar_lifetime_params ())*)>,
            #(#cache_fields_def),*
        }

        impl<'input: 'rule, 'rule #(, #grammar_lifetime_bounds)*> ParseState<'input, 'rule #(, #grammar_lifetime_params)*> {
            fn new() -> ParseState<'input, 'rule #(, #grammar_lifetime_params)*> {
                ParseState {
                    _phantom: ::core::marker::PhantomData,
                    #(#cache_fields: ::peg::hashbrown::HashMap::new()),*
                }
            }
        }
    }
...
fn compile_rule(context: &Context, rule: &Rule) -> TokenStream {
    ...
    let fn_body = match &rule.cache {
        ...
        Some(cache_type) => {
            ...
            let cache_ty = format_ident!("Cached{}", rule.name);
            ...
            let borrowed_key = if rule.params.is_empty() {
                quote_spanned!(span=> __pos)
            } else {
                let param = rule.params.iter().map(|param| &param.name);
            ...
            let key = if rule.params.is_empty() {
                ...
                let param = rule.params.iter().map(|param| &param.name);
                quote_spanned!(span=> #cache_ty(__pos #(, ::peg::Cacheable::to_cached(&#param))*, ::core::marker::PhantomData))
            };

            match cache_type {
                Cache::Simple => quote_spanned! { span =>
                    if let Some(entry) = __state.#cache_field.get(&#borrowed_key) {
                        #cache_trace
                        return entry.clone();
                    }
